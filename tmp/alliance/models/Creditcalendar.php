...
class Creditcalendar extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasMany(User::className(), ['id' => 'user_id'])->viaTable('{{%calendar_responsibles}}', ['user_id' => 'id']);
    }
...
//    }
    ...
    private $_responsiblesArray;
    public function getCreditcalendarResponsiblesArray()
    {
        if ($this->_responsiblesArray === null) {
            $this->_responsiblesArray = $this->getUser()->select('id')->column();
        }
        return $this->_responsiblesArray;
    }
    public function setCreditcalendarResponsiblesArray($value)
    {
        $this->_responsiblesArray = (array)$value;
    }
    ...
    {
        $currentResponsiblesIds = $this->getUser()->select('id')->column();
        $newResponsiblesIds = $this->getResponsiblesArray();
        foreach (array_filter(array_diff($newResponsiblesIds, $currentResponsiblesIds)) as $responsibleId) {
            /** @var user $responsible */
            if ($responsible = CreditcalendarResponsibles::findOne($responsibleId)) {
                $this->link('user', $responsible);
            }
        }
        foreach (array_filter(array_diff($currentResponsiblesIds, $newResponsiblesIds)) as $responsibleId) {
            /** @var user $responsible */
            if ($responsible = CreditcalendarResponsibles::findOne($responsibleId)) {
                $this->unlink('user', $responsible, true);
            }
        }
    }
